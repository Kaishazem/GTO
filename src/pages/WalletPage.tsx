...
function validateAddress(
  ...
  if (network === "TRC20") {
    ...
    if (!isValidERC20(addr))
      return "Invalid ERC20 address — must start with 0x and be 42 characters";
  }
  return null;
}
...
export default function WalletPage() {
  ...
  const minAmount =
    currency === "USDT" ? (settings?.usdtMin ?? 10) : (settings?.usdcMin ?? 10);
  ...
  useEffect(() => {
    if (settings) {
      if (currency === "USDT" && !usdtEnabled && usdcEnabled)
        setCurrency("USDC");
      if (currency === "USDC" && !usdcEnabled && usdtEnabled)
        setCurrency("USDT");
    }
  }, [settings]);
      ...
      try {
        ...
        const [exactSnap, lowerSnap, wSnap] = await Promise.all([
          getDocs(
            query(
              collection(db, "users"),
              where("trc20Address", "==", walletAddress),
            ),
          ),
          getDocs(
            query(
              collection(db, "users"),
              where("walletAddressLower", "==", addrLower),
            ),
          ),
          getDocs(
            query(
        ...
        if (takenByOther && s) {
          toast({
            ...
            description:
              "This wallet is already linked to another account. Please use a different wallet address.",
            variant: "destructive",
          });
          return;
        }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (amountNum < minAmount) {
      toast({
        title: "Below minimum",
        description: `Minimum withdrawal is $${minAmount}`,
        variant: "destructive",
      });
      return;
    }
    if (netAmount <= 0) {
      toast({
        title: "Net amount too low",
        description: "The net amount after fees must be greater than zero",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "✅ Submitted",
        description: "Your withdrawal request is under review",
      });
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-5">
          ...
          <p className="text-3xl font-bold text-white">
            {(profile?.pendingBalance || 0).toFixed(5)}
          </p>
          ...
          <div>
            ...
            <p className="text-amber-400/70 text-sm mt-0.5">
              ...
              <span className="font-bold text-amber-300">
                ${needMore.toFixed(5)} more
              </span>{" "}
              to reach the ${minAmount} minimum.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-white/40">
              Minimum ${minAmount} • Subject to fees
            </p>
          </div>
        </div>
            ...
            <div className="flex gap-2">
              {(["TRC20", "ERC20"] as Network[]).map((n) => (
                ...
                >
                  {n}{" "}
                  {n === "TRC20"
                    ? `(Gas $${settings?.usdcTrc20Gas ?? 1})`
                    : `(Gas $${settings?.usdcErc20Gas ?? 5})`}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 mb-5 flex gap-2">
          ...
          <p className="text-xs text-amber-300">
            Double-check your {network} wallet address. Incorrect addresses
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div>
            <label className="text-sm font-medium text-white/80 block mb-1.5">
              ...
              {network === "TRC20" ? "(starts with T)" : "(starts with 0x)"}
            </label>
            <div className="flex gap-2">
              <Input
                value={walletAddress}
                ...
                className={cn(
                  "bg-white/10 border-white/20 text-white placeholder:text-white/30 focus:border-emerald-400 font-mono text-sm flex-1",
            ...
            {addrError && walletAddress && (
              <p className="text-xs text-red-400 mt-1">{addrError}</p>
            )}
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-white/40 mt-1">
              Min: $
              {minAmount.toLocaleString("en-US", { minimumFractionDigits: 2 })}{" "}
              • Available: $
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="font-bold text-emerald-400">
                      ${w.netAmount.toFixed(5)}
                    </p>
                    <p className="text-xs text-white/30">
                      {w.currency} net payout
                    </p>
                    {w.netAmount !== w.amount && (
                      <p className="text-xs text-white/20">
                        requested: ${w.amount.toFixed(5)}
                      </p>
                    )}
                  </div>
                </div>
