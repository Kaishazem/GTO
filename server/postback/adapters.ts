...
export const ADAPTERS: NetworkAdapter[] = [
  ...
  //   engine.ts (rawParams inspection) because they require logic beyond static
  //   statusMap/params config.
  // Status: approved|pre_approved→PAY; pending→RECORD only; rejected→no-pay;
  //   unknown/missing→fallback approved + warn (handled in engine).
  // convId: transaction_id primary; program_config_id + '_' + ml_sub1 fallback
  //   (computed in engine when transaction_id is absent).
  {
    id: "mylead",
    displayName: "MyLead",
    detect: [
